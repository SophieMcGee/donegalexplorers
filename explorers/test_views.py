...
from explorers.models import (
    Event, Calendar, Comment, Notification, UserProfile
)
...
class TestViews(TestCase):
    ...
    def test_event_update_view(self):
        response = self.client.get(
            reverse('edit_event', kwargs={'pk': self.event.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_event_delete_view(self):
        response = self.client.get(
            reverse('delete_event', kwargs={'pk': self.event.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_save_event_to_calendar_view(self):
        ...
        self.assertTrue(
            Calendar.objects.filter(user=self.user, event=self.event).exists()
        )

    def test_remove_event_from_calendar_view(self):
        Calendar.objects.create(
            user=self.user, event=self.event, date=self.event.start_date
        )
        response = self.client.post(
            reverse(
    ...
    def test_mark_notification_as_read_view(self):
        notification = Notification.objects.create(
            user=self.user, message="New notification"
        )
        response = self.client.post(
            reverse(
